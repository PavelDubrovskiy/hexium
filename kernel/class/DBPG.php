...
class DB{
	...
	public function __construct(){
		try{
			DB::$pdo = new PDO('pgsql:host='.BDHOST.';dbname='.BDNAME.'', BDLOGIN, BDPASS, array(PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES \'UTF8\''));
	...
	public function getRow($sql,$exec=0){
		$sql=DB::prefix($sql,$exec);
		try{
			DB::$pdo->setAttribute(PDO::ATTR_EMULATE_PREPARES, true);
			$row=DB::$pdo->query($sql);
	...
	public function exec($sql){
		try{
			$sql=DB::prefix($sql,1);
			DB::$pdo->exec($sql);
			if(strpos($sql,'INSERT')!==false) return DB::$pdo->lastInsertId();
		}catch(PDOException $e){
			die(DB::error($sql,debug_backtrace()));
		}
	}
